...
class CarlaSumoGym(gym.Env):
    ...
    def connect_server_client(self, display = True, rendering = True, synchronous = True, town = 'Town11', fps = 10.0, sumo_gui = False):
        ...
        while True:
            try:
                ...
                # self.client = carla.Client('127.0.0.1', 2000, 10).load_world(town)
                self.client = carla_sim.client
                self.world = self.client.get_world()
                self.map = self.world.get_map()

                # if self.world.get_map().name != town:
                #     carla.Client('127.0.0.1', 2000, 10).load_world(town)
                #     while True:
                #         try:
        ...
        net_file = os.path.join(basedir, 'sumo_config/net/Town11.net.xml')#current_map.name
        cfg_file = os.path.join(basedir, 'sumo_config/Town11.sumocfg')
    ...
    def spawn_ego_vehicle(self, position, type_id, max_speed = 5.0):
        ...
        traci.vehicle.setMaxSpeed(typeID = 'ev', speed = max_speed)
    ...
    def get_ego_vehicle_transform(self):
        ego_vehicle = self.world.get_actors().filter('vehicle.tesla.cybertruck')
