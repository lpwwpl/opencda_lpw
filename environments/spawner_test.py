...
# def main():
#
#     # carla_sim = CarlaSimulation('127.0.0.1', 2000, 0.1) # host, port, step_length
...
def run_step():
    ...
    odr_map = world.get_map()
    spawning_landmark = odr_map.get_crosswalks()
    offset_location = carla.Location(spawning_landmark[0].x + 5,
                                     spawning_landmark[0].y + 5,
                                     spawning_landmark[0].z + 5)
    # loc = offset_location

    # transform = carla.Transform(
    #     carla.Location(
    #     x=spawning_landmark[0].x,
    #     y=spawning_landmark[0].y+5,
    #     z=spawning_landmark[0].z),
    #     carla.Rotation(
    #     pitch=0,
    #     yaw=0,cd
    #     roll=0))
    ...
    pedestrian = world.spawn_actor(bp, end)
    ...
    # controller.direction.y = -10
    # controller.direction.x = 0
    # controller.direction.z = 0
    ...
    controller.go_to_location(end.location)#end.location)  #world.get_random_location_from_navigation()
    controller.set_max_speed(1 + random.random())

    spectator = world.get_spectator()
    location_walker = pedestrian.get_location()
    spectator.set_transform(carla.Transform(location_walker + carla.Location(z=30), carla.Rotation(pitch=-90)))
