...
def color_pcd_get(pcd, projected_lidar, rgb_img: np.ndarray):
    img_h, img_w = rgb_img.shape[:2]

    keep_idx = (projected_lidar[:, 0] > 0) * (projected_lidar[:, 0] < img_w) * (projected_lidar[:, 1] > 0) * (projected_lidar[:, 1] < img_h)
    # retrive color points

    # projected_lidar = np.fliplr(projected_lidar)
    projected_lidar = projected_lidar[keep_idx].astype(np.int32)
    w_idx = projected_lidar[:, 0]
    h_idx = projected_lidar[:, 1]
...
def o3d_pointcloud_encode_color(raw_data, point_cloud, projected_lidar, rgb_img):
    ...
    int_color = color_pcd_get(points, projected_lidar,rgb_img)

    point_cloud.points = o3d.utility.Vector3dVector(points)
    point_cloud.colors = o3d.utility.Vector3dVector(int_color)

    return int_color,points
...
def o3d_visualizer_with_color(vis, count, point_cloud,  objects):
    # pt1 = o3d.geometry.PointCloud()
    # pt1.points = o3d.utility.Vector3dVector(point_cloud.reshape(-1, 3))
    if count == 2:
...
def o3d_camera_lidar_fusion(objects,
    ...
    for detection in yolo_bbx:
        ...
        if is_vehicle_cococlass(label):
            ...
            obstacle_vehicle.set_v_id(v_id)
